"""Loader helpers for the ragged all-positions cache (CB_activations/<tag>_allpos/).

One source of truth for turning the per-layer ragged arrays + span/offset meta into the
per-example tensors the positional probes need (full-mean, option-span pool, learned-attention),
so every probe pools over IDENTICAL token spans. Ported from the RMU harness; only ALLPOS_DIR
changes (CB lives under /workspace/CB_activations).
"""
import os

import numpy as np

ALLPOS_DIR = "/workspace/CB_activations"
...
def load_layer(tag, L):
    """Full per-layer ragged array (total_tokens, hidden) in RAM."""
...
def _flat_rows(meta, i, span):
    ...
    'options' (union of 4 option lines), 'question', 'colon', or 'all'."""
