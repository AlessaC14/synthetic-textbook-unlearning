...
def _reach(adj, starts):
    """All nodes reachable from `starts` over adjacency `adj` (incl. starts)."""
...
def cmd_split(args):
    """Partition the graph around target claim(s) for an unlearning experiment.

    Edges are node -> prerequisite (depends_on). From a set of TARGETS:
      forget set = targets + everything that (transitively) depends ON a target
                   — the consequence-closure; these only make sense given the target.
      retain set = the targets' transitive prerequisites that are NOT themselves
                   in the forget set — general foundations you want to keep intact.
      crossing   = edges with exactly one endpoint in the forget set. This is the
                   collateral estimate: the larger it is, the more entangled the
                   target is with knowledge you intend to preserve.
    """
    nodes = load_graph(args.graph)
    ids = {n["id"] for n in nodes}
    targets = [t.strip() for t in args.targets.split(",") if t.strip()]
    if args.targets_file:
        targets += [t for t in json.load(open(args.targets_file)) if t]
    bad = [t for t in targets if t not in ids]
    if bad:
        sys.exit(f"Unknown target id(s): {bad}")
    if not targets:
        sys.exit("No targets given (use --targets C001,C002 or --targets-file ids.json).")

    deps = {n["id"]: list(n.get("depends_on", [])) for n in nodes}   # node -> prereqs
    dependents = defaultdict(list)                                   # prereq -> dependents
    for nid, ds in deps.items():
        for d in ds:
            if d in ids:
                dependents[d].append(nid)

    forget = _reach(dependents, targets)                # targets + their consequences
    prereq_closure = _reach(deps, targets)              # targets + their foundations
    retain = prereq_closure - forget  # foundations not swept into the forget set

    crossing = [(nid, d) for nid in ids for d in deps[nid]
                if d in ids and ((nid in forget) ^ (d in forget))]
    untouched = ids - forget - retain
    ...
    out = {
        ...
        "crossing_edges": [{"from": a, "to": b} for a, b in crossing],
        "forget_claims": {i: by_id[i]["claim"] for i in sorted(forget)},
        "retain_claims": {i: by_id[i]["claim"] for i in sorted(retain)},
    }
    json.dump(out, open(args.out, "w"), indent=2)
    print(f"targets {targets}: forget={len(forget)} retain={len(retain)} "
          f"untouched={len(untouched)} | crossing edges (collateral est.)={len(crossing)}")
    print(f"  forget : {sorted(forget)}")
    print(f"  retain : {sorted(retain)}")
    if crossing:
        print("  collateral risk — forget nodes wired to kept knowledge:")
        for a, b in crossing:
            side = "depends on kept" if a in forget else "kept depends on forget"
            print(f"    {a} -> {b}  ({side})")
    print(f"-> {args.out}")
...
def main():
    ...
    p = sub.add_parser("split"); p.add_argument("--graph", required=True); p.add_argument("--targets", default="", help="comma-separated claim ids, e.g. C001,C003"); p.add_argument("--targets-file", dest="targets_file", help="JSON list of ids"); p.add_argument("--out", default="split.json"); p.set_defaults(fn=cmd_split)
