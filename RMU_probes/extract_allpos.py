"""Permanent all-positions extraction (fixes the dropped "cache all positions" step).

Writes a position-indexable RAGGED cache per model so EVERY downstream positional probe
(option-span pool, learned-attention, any future span probe) runs off this one cache and we
never re-extract again. Ragged (concatenated real tokens) not dense-padded: 42 GB/model vs
297 GB/model, because one prompt is 862 tokens while the median is 118.

Output dir  activations/<tag>_allpos/ :
  ...
  meta.npz :
    labels (N,) int64
    offsets (N+1,) int64        -- example i occupies flat rows [offsets[i] : offsets[i+1]]
    lengths (N,) int64          -- real-token count per example
    q_span (N,2) int64          -- LOCAL token [start,end) of the question
    opt_spans (N,4,2) int64     -- LOCAL token [start,end) per option line A/B/C/D
    colon_local (N,) int64      -- LOCAL token index of the ':' (== lengths-1)
    behav_pred (N,) int64, behav_acc float64
    model_path, n_layers, total_tokens
  (flat row for a LOCAL index t in example i = offsets[i] + t)

Usage:
  python extract_allpos.py --model_path /workspace/models/wmdp/Zephyr_RMU --tag rmu
"""
import argparse
import os
...
from prompt_utils import (LETTER_TOKEN_IDS, TOKENIZER_DIR, build_prompt,
                        build_prompt_with_spans, load_wmdp_bio)


def char_span_to_tokens(offsets, c0, c1):
    """LOCAL token [start,end) whose char ranges overlap [c0,c1). offsets: list[(a,b)] per token
    (special/pad tokens are (0,0) and never overlap a positive span)."""
...
def main():
    ...
    ap.add_argument("--tag", required=True, help="base | rmu (names the output dir)")
    ap.add_argument("--outdir", default="/workspace/activations")
    ...
    # --- pass 1: token lengths + per-example spans (CPU tokenize w/ offsets) ---
    lengths = np.zeros(N, dtype=np.int64)
    q_span = np.zeros((N, 2), dtype=np.int64)
    opt_spans = np.zeros((N, 4, 2), dtype=np.int64)
    for i, p in enumerate(prompts):
        enc = tok(p, add_special_tokens=True, return_offsets_mapping=True)
