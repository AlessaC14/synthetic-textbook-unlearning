...
def call_llm(system, user, model, max_retries=4):
    """Single JSON-mode call with exponential backoff. Reused by Pass 2."""
    client = get_client()
    last_err = None
    for attempt in range(max_retries):
        try:
            resp = client.chat.completions.create(
                model=model,
                temperature=0,  # deterministic-ish, for reproducibility
